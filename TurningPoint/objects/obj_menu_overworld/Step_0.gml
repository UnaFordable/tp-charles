...
if(m_state == MENU.STATS){
	if(instance_exists(obj_item_use))with(obj_item_use){instance_destroy();}
	if(instance_exists(obj_item_button))with(obj_item_button){instance_destroy();}
	if(instance_exists(obj_perks_button))with(obj_perks_button){instance_destroy();}
}
else if(m_state == MENU.ITEMS){
	...
	if(!instance_exists(obj_item_use))instance_create_depth(global.cam_bottom_x + 176, global.cam_bottom_y +144, depth-1, obj_item_use);
}
